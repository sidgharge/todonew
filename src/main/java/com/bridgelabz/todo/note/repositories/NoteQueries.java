...
public interface NoteQueries {
	...
	String FETCH_ALL_BY_USER_ID = "SELECT note.id, note.title, note.body, note.created_at, note.updated_at, note_extras.color, note_extras.is_archived, note_extras.is_pinned, note_extras.is_trashed, note_extras.reminder, note_image_urls.image_urls, label.id as l_id, label.name as l_name, user.id as o_id, user.firstname as o_firstname, user.lastname as o_lastname, user.email as o_email, user.profile_url as o_profile_url, c.id as c_id, c.firstname as c_firstname, c.lastname as c_lastname, c.email as c_email, c.profile_url as c_profile_url from note_extras LEFT JOIN note on note_extras.note_id = note.id LEFT JOIN note_image_urls ON note.id = note_image_urls.note_id LEFT JOIN note_label ON note_extras.id = note_label.neid LEFT JOIN label ON label.id = note_label.label_id LEFT JOIN user ON note.owner_id = user.id LEFT JOIN note_extras as cne ON note.id = cne.note_id AND cne.owner_id <> note.owner_id LEFT JOIN user as c ON cne.owner_id = c.id  WHERE note_extras.owner_id = :owner_id;";

	String DELETE_NOTE_IMAGE = "DELETE FROM note_image_urls WHERE note_id = :note_id AND image_urls = :image_urls";
}
